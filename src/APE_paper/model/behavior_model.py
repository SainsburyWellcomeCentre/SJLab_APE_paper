...
def get_df_of_behavior_model_by_animal(df_to_plot, mouse_max_perf):
    ...
    for counter, animal in enumerate(ans_list):
        ...
        cost_func = lambda x: np.mean(np.abs(model_utils.sigmoid_func(xdatasc, x[0], x[1], x[2]) - ydatasc))
...
def get_steepest_point_of_slope(fit_df):
    ...
    for animal in ans_list:
        m_point = opt.fmin(lambda x: -model_utils.der_sig(x, *[fit_df[fit_df.AnimalID==animal].maximum_performance.iloc[0],
